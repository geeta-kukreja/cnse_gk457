...
//						   on how to use it.
//
//	 YOUR ANSWER: It defines variables to store the values of the command line flags.
// The flag package is used to associate these variables with specific command line flags. Each flag is defined with its name, default value, and a description of its usage.
// The flag.Parse() function is called to parse the command line arguments and assign values to the corresponding variables.
// The appOpt variable is initialized with the value INVALID_APP_OPT.
// If no flags are set, the function displays the usage information and returns an error.
// It iterates over the set flags using flag.Visit(). Based on the flag's name, it assigns the appropriate value to the appOpt variable (e.g., LIST_DB_ITEM, QUERY_DB_ITEM, etc.).
// Finally, it checks if the appOpt value is valid or implemented. If not, it displays an error message along with the usage information and returns an error.
...
func main() {
	...
	case CHANGE_ITEM_STATUS:
		...
		fmt.Println("implemented for extra credit")
		fmt.Println("Ok")
	default:
