...
//
// ANSWER: Making the fields in the `ToDo` struct lowercase and unexported is a deliberate design choice.
// It helps to encapsulate the internal state of the `ToDo` struct, ensuring that its fields cannot be accessed directly from outside the `db` package. This encapsulation promotes better control over how the struct is used and modified.
type ToDo struct {
	toDoMap    DbMap
	dbFileName string
}
...
func (t *ToDo) GetItem(id int) (ToDoItem, error) {
	...
	// Load the database into the private map in our struct
	err := t.loadDB()
	if err != nil {
		return ToDoItem{}, err
	}
